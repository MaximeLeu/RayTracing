...
if __name__ == "__main__":
    ...
    diff_point_1, _ = geom.diffraction_point_from_origin_destination_and_edge(
        A, B, vertice
    )
    (
        diff_point_2,
        ...
        A, B, [screen.get_parametric()], vertice
    )

    diff_line_1 = np.row_stack([A, diff_point_1, B])
    diff_line_2 = np.row_stack([A, diff_point_2, B])
    plot_utils.add_line_to_3d_ax(
        ax, diff_line_1, color="b", linestyle="--", label="single diffraction"
    )
    plot_utils.add_line_to_3d_ax(
        ax, diff_line_2, color="r", linestyle="--", label="reflection + diffraction"
    )

    ref_point, sol = geom.reflexion_points_from_origin_destination_and_planes(
        A, B, [screen.get_parametric()]
    )
    ref_line = np.row_stack([A, ref_point, B])
    plot_utils.add_line_to_3d_ax(ax, ref_line, alpha=0.5, label="single reflection")

    ref_points, sol = geom.reflexion_points_from_origin_destination_and_planes(
        A, B, [screen.get_parametric(), face.get_parametric()]
    )
